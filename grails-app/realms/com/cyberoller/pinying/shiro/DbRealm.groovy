...
class DbRealm {
    ...
    def authenticate(authToken) {
		...
		def account = new SimpleAccount(username, user.passwordHash, bytesource, "DbRealm")

		//检查用户是否禁用
        if(user.isBlocked){
        	log.info "User [${username}] is blocked."
        	throw new AccountBlockedException("User [${username}] is blocked.")
        }

        if (!credentialMatcher.doCredentialsMatch(authToken, account)) {
            log.info "Invalid password (DB realm)"
            //密码不匹配，记录错误次数
            def errortimes = user.errortimes
            user.errortimes = errortimes + 1
        ...
        if(!user.isActive){
        	...
        	throw new AccountInactiveException("User [${username}] is inactive.")
        }
